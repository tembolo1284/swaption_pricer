...
double HullWhiteModel::priceSwaption(double swapRate, double strikeRate, double maturity, double tenor) const {
...
std::vector<double> HullWhiteModel::generateShortRatePath(double initialRate, double maturity, int steps) const {
    ...
    for (int i = 1; i <= steps; ++i) {
        double drift = meanReversion * (0 - rates[i - 1]) * dt;
